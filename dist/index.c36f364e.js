...
function showTitle(titleNumber) {
    titles.forEach((title)=>{
        if (titles[titleNumber] != title) title.classList.replace("appear", "disappear");
    });
    if (!titles[titleNumber].classList.contains("appear")) {
        titles[titleNumber].innerHTML = songs[titleNumber].title;
        if (window.innerWidth >= 1080) titles[titleNumber].style.setProperty("--song-size", sizes[Math.floor(Math.random() * sizes.length)].size);
        else titles[titleNumber].style.setProperty("--song-size", mobileSizes[Math.floor(Math.random() * mobileSizes.length)].mobileSize);
        titles[titleNumber].style.setProperty("--coord-top", `${(Math.floor(Math.random() * 17) + 1) * 5}%`);
        titles[titleNumber].style.setProperty("--coord-right", `${(Math.floor(Math.random() * 14) + 1) * 5}%`);
...
function totalSeparateDate() {
    ...
    if (allSeconds >= 0) return waitTime = [
        {
...
function changeTimer() {
    let waitTime1 = totalSeparateDate();
    if (waitTime1) {
        if (waitTime1[0].days != timerDays.innerHTML) timerDays.innerHTML = waitTime1[0].days;
        if (waitTime1[0].hours != timerHours.innerHTML) timerHours.innerHTML = waitTime1[0].hours;
        if (waitTime1[0].minutes != timerMinutes.innerHTML) timerMinutes.innerHTML = waitTime1[0].minutes;
        timerSeconds.innerHTML = waitTime1[0].seconds;
    }
}
...
async function blinkElementToHide(element, timeInDiffState, elementsTransitionTime, exist) {
    ...
    console.log("disappear");
    await sleep(1000);
    if (exist == true) element.addEventListener("transitionend", async function() {
        console.log("hidden");
        element.classList.add("hidden");
    });
    await sleep(timeInDiffState);
...
range.addEventListener("input", async function() {
    ...
    console.log(value);
    if (value != 0 && value != 100) showTitle(Math.ceil(value / 9) - 1);
    else if (range.value == 100) changeToFinalScreen();
    else hideAllTitles();
    ...
    async function massEffect() {
        ...
        for(i = 0; i < 850; i++){
            ...
            paragraph.style.setProperty("--coord-top", `${Math.floor(Math.random() * 120) - 10}%`);
            paragraph.style.setProperty("--coord-right", `${Math.random() * 120 - 10}%`);
            massive.append(paragraph);
            await sleep(ms);
            paragraph.classList.remove("disappear");
            if (i == 800) body.classList.replace("oldBack", "newBack");
            if (ms != 10) ms = ms - 5;
        }
